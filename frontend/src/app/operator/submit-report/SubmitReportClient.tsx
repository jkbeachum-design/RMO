...
import { DEFAULT_LICENSE, PILOT_NAME } from '@/lib/constants';
...
export default function SubmitReportClient({ userName }: { userName: string }) {
  ...
  const [operatorName, setOperatorName] = useState(PILOT_NAME);
  const [licenseId, setLicenseId] = useState(DEFAULT_LICENSE);
  ...
  useEffect(() => {
    ...
    async function loadContext() {
      ...
      setError('');
      try {
        const res = await fetch(`/api/operator-context?license_number=${licenseId}`);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch(`${backend}/api/submit-report`, {
        method: 'POST',
        body: formData
      });
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-8">
          <section className="border-l-4 border-teal-700 bg-white p-5 pl-4 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              <label className="block text-sm">
                <span className="mb-1 block font-medium text-slate-700">License</span>
                <select
                  value={licenseId}
                  onChange={(e) => setLicenseId(e.target.value)}
                  className="w-full rounded border border-slate-300 px-3 py-2"
                >
                  <option value="836089">Beachum Construction (#836089)</option>
                  <option value="1160775">Vanguard Property Maintenance (#1160775)</option>
                </select>
              </label>
              {loadingContext ? (
