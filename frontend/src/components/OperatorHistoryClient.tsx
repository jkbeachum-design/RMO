  ...
  async function saveEdit() {
    ...
    if (draft.sub_company.trim()) {
      const subRow = {
        ...
        trade: draft.sub_trade || undefined,
    ...
    try {
      const res = await fetch(`/api/operator/logs/${editingLog.id}`, {
        method: 'PATCH',
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <p className="mt-2 text-slate-600">
        Recent check-ins{entityName ? ` for ${entityName}` : ''}. Open{' '}
        <strong>Edit report</strong> to fix typos or correct details — risk flags recalculate on
        save.
      </p>
